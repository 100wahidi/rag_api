...
async def process_offer(extraction_input: ExtractionInput, username: str = Depends(get_current_user)):
    try:
        text_extraction = ExtractionService(api_key=settings.MISTRAL_API_KEY).extract(extraction_input.offer)
